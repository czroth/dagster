...
def hash_named_tuple(named_tuple: NamedTuple) -> int:
    """Hash a NamedTuple that contains mutable data structures.
    ...
    Example:
        .. code-block:: python
            ...
            def __hash__(self):
                ...
                return self._hash
    """
    hashable_elements = tuple(make_hashable(e) for e in named_tuple)
    return hash(hashable_elements)
...
def make_hashable(value: Any) -> Any:
    ...
    elif isinstance(value, (list, tuple, set)) and not is_named_tuple_instance(value):
...
def _hash_collection(value: object) -> int:
    if isinstance(value, dict):
        return hash(tuple(sorted((k, _hash_collection(v)) for k, v in value.items())))
    elif isinstance(value, list):
        return hash(tuple(_hash_collection(x) for x in value))
    else:
        return hash(value)
